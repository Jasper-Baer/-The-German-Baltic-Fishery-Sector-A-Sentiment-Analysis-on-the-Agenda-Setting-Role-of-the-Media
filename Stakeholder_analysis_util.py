...
def stakeholder_analysis(data, common_pers, common_orgs, stakeholders_pers_list, stakeholders_orgs_list, start_year, end_year, quota_dates):
    ...
    def filter_stakeholders(data, common_pers, common_orgs, stakeholders_pers_list, stakeholders_orgs_list, start_year, end_year):
        ...
        # text_pers = text_pers.groupby('Name').agg({
        #         'ID': 'sum',  # This concatenates the lists of IDs
        # }).reset_index()
        
        # id_pers = id_pers.groupby('Name').agg({
        #         'ID': 'sum',  # This concatenates the lists of IDs
        # }).reset_index()
        
        # text_org = text_org.groupby('Name').agg({
        #         'Sentences': 'sum',  
        # }).reset_index()
        
        # id_org = id_org.groupby('Name').agg({
        #         'ID': 'sum', 
        # }).reset_index()
        
        # #text_org['ID'] = id_org['ID']
        
        # text_org = text_org.explode('Sentences')
        
        # id_org = id_org.explode('ID')
        
        # text_org['ID'] = id_org['ID']
        
        #text_org.to_excel(r'D:\Studium\PhD\Github\Fischerei - Master\Sentiment-Analysis-Western-Baltic-Sea\Topic and Stakeholder Analysis\text_id_orgs.xlsx')
