...
def test(model, bert_type, train_on_gpu, test_dataloader, num_classes = 3):
    ...
    # accuracy over all test data
  #  test_pre = precision_score(true_label, pred_label)
    
    # accuracy over all test data
   # test_rec = recall_score(true_label, pred_label)
    ...
    print('Test loss: {:.3f}'.format(avg_test_loss),
         ...
         # 'Test precision: {:.3f}'.format(test_pre),
         # 'Test recall: {:.3f}'.format(test_rec),
    ...
    plt.savefig('D:\Studium\PhD\BERT_results.png', format='png', dpi=2000)
    plt.show()
