...
PATH = r"D:\Studium\PhD\Fischerei\Raw Data\Complete Data"
PATH_LEXISNEXIS = r"D:\Studium\PhD\Fischerei\Raw Data\LexisNexis"
PATH_SZ = r'D:\Studium\PhD\Fischerei\Raw Data\SZ'
PATH_OZ = r'D:\Studium\PhD\Fischerei\Raw Data\Ostsee Zeitung'
...
def load_fishery_data():
    ...
    Returns:
        pd.DataFrame: Combined data from LexisNexis, Süddeutsche Zeitung, and Ostsee Zeitung.
    """
    ...
    for j in range(2004, 2022):
        extra_info = pd.read_excel(os.path.join(PATH, 'sentiment analysis, newspaper, 10022023 FINAL.xlsx'), sheet_name=str(j))
    ...
    for j in range(2008,2022):
        articles = docx2txt.process(PATH + '\\' + str(j) + '\\' + 'np.' + str(j)+", all, txt.docx")  
        extra_info = pd.read_excel(PATH + "\\" + 'sentiment analysis, newspaper, 10022023 FINAL.xlsx', sheet_name=str(j))
    ...
    for year in range(2004, 2022):
        try:
            extra_info_SZ = pd.concat([extra_info_SZ, pd.read_excel(PATH + "\\" + 'sentiment analysis, newspaper, 10022023 FINAL.xlsx', sheet_name=str(year))])
        except ValueError:
            continue
    ...
    for year in [2011, 2012, 2014, 2015, 2016, 2017, 2019, 2020, 2021]:
        extra_info_oz = pd.read_excel(PATH + "\\" + 'sentiment analysis, newspaper, 10022023 FINAL.xlsx', sheet_name=str(year))
    ...
    number_journals = pd.read_excel("D:\Studium\PhD\Fischerei\journal_occur_small_journals_ordered.xlsx", header = None)   
